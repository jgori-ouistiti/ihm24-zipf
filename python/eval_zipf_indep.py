...
if __name__ == '__main__':
    ...
    zipf_seq = gen_zipf_seq(USER0_n_com, USER0_seq_len, s = 1.88)


    fig, axs = plt.subplots(nrows = 1, ncols = 3, width_ratios=[1, 1, 1.5], figsize = (15,6))

    trans_matrix, count_matrix, mapping  = transition_matrix(zipf_seq, sort_by_most_frequent=False)         
    ...
    observed_sparsity = numpy.sum(trans_matrix < 0.01)/trans_matrix.shape[0]**2

    axs[2].set_title(f'Sparsity={observed_sparsity:.2f}')


    ax, fit_results_zip_freq = zipf_plot(zipf_seq, mode = "sequence", ax = axs[0], fit = True, ax_kws = {'label': 'Indep. Zipf'})
    ...
    # plt.savefig(f'img/zipf_indep_10808.pdf')
    plt.show()
    plt.close()
